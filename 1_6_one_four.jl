...
ONE_FOUR_SIX= Model(Gurobi.Optimizer)


@variable(ONE_FOUR_SIX, power_DA[1:T]>=0)

@variable(ONE_FOUR_SIX, delta[1:T,1:O])           # if system is in excess (need down regulation), in every case it will pay 0.9*DA_price 
@variable(ONE_FOUR_SIX, delta_more[1:T,1:O]>=0)   # no matter whether the balancing provided by the wind farm is helping the system or not
@variable(ONE_FOUR_SIX,delta_less[1:T,1:O]>=0)    # 1.2 if the system is in deficit
@variable(ONE_FOUR_SIX, epsi)
@variable(ONE_FOUR_SIX, mi[1:O]>=0)


@objective(ONE_FOUR_SIX, Max, sum(prob_in[o]*(scenarios_200[t,DA_price,o]*power_DA[t] + Balancing_price_in[t,o]*delta[t,o]) for t=1:T, o=1:O) + beta[B]*(epsi - 1/(1-alpha)*sum(prob[o]*mi[o] for o=1:O)))

@constraint(ONE_FOUR_SIX,[t=1:T], power_DA[t]<=wind_farm_CAP)
@constraint(ONE_FOUR_SIX, [t=1:T,o=1:O], delta[t,o] == scenarios_200[t,DA_wind,o] - power_DA[t])
#@constraint(ONE_SIX, [t=1:T,o=1:O], delta[t,o] == delta_more[t,o] - delta_less[t,o])
@constraint(ONE_FOUR_SIX,[o=1:O],-sum(scenarios_200[t,DA_price,o]*power_DA[t] + Balancing_price_in[t,o]*delta[t,o] for t=1:T) + epsi - mi[o]<=0)

optimize!(ONE_FOUR_SIX)
