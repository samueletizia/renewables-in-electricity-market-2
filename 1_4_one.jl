...
FOUR_ONE= Model(Gurobi.Optimizer)


@variable(FOUR_ONE, power_DA[1:T]>=0)

@variable(FOUR_ONE, delta[1:T,1:O])           # if system is in excess (need down regulation), in every case it will pay 0.9*DA_price 
@variable(FOUR_ONE, delta_more[1:T,1:O]>=0)   # no matter whether the balancing provided by the wind farm is helping the system or not
@variable(FOUR_ONE,delta_less[1:T,1:O]>=0)    # 1.2 if the system is in deficit
@variable(FOUR_ONE, epsi)
@variable(FOUR_ONE, mi[1:O]>=0)

@objective(FOUR_ONE, Max, sum(prob[o]*(scenarios_200[t,DA_price,o]*power_DA[t] + Balancing_price[t,o]*delta[t,o]) for t=1:T, o=1:O) 
                    + 1*(epsi - 1/(1-alpha)*sum(prob[o]*mi[o] for o=1:O)))

@constraint(FOUR_ONE,[t=1:T], power_DA[t]<=wind_farm_CAP)
@constraint(FOUR_ONE, [t=1:T,o=1:O], delta[t,o] == scenarios_200[t,DA_wind,o] - power_DA[t])
@constraint(FOUR_ONE,[o=1:O],-sum(scenarios_200[t,DA_price,o]*power_DA[t] + Balancing_price[t,o]*delta[t,o] for t=1:T) + epsi - mi[o]<=0)
